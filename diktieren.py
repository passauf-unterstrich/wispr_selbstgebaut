...
# ─────────────────────────────────────────
    ...
    def starte_aufnahme(self):
        ...
        if _app_instanz is not None:
            _app_instanz._letzter_chunk_ende = ""
        ...
        try:
            ...
            if _app_instanz is not None and _app_instanz._letzter_chunk_ende:
                ...
                for laenge in range(min(len(letzter), 40), 4, -1):
                    kandidat = letzter[-laenge:].lower()
                    if text_lower.startswith(kandidat):
                        # Kappen (Original-Case erhalten indem wir vom Original schneiden)
                        original_lstripped = text.lstrip()
                        vorspann = text[:len(text) - len(original_lstripped)]
                        text = vorspann + original_lstripped[laenge:].lstrip()
                        _debug_log(f"Nahtstellen-Dedup: {laenge} Zeichen gekappt")
                        break
            if text.strip():
                fuege_text_ein(text)
    ...
    def _vad_cutter(self):
        ...
        while not self._cutter_stop.is_set():
            ...
            try:
                ...
                # Kein Overlap – sonst doppelte Wörter an Nahtstellen
    ...
    def _finde_schnittstelle(self, audio_np, dauer_sek):
        ...
        for seg in reversed(segmente):
            ...
            if ziel_sample <= ende <= max_sample:
                schnitt = ende + int(0.3 * SAMPLE_RATE)  # kurzes Padding + Dedup fängt Reste  # großzügig damit leise Satzenden mitkommen
